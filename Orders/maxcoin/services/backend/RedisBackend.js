...
class RedisBackend {
  ...
  // the below function doesn't need to be async because under the new Redis, we'll just
  // return the client object and the actually connection will take place the
  // first time we do a comment on the database. Connection is created lazily
  connect() {
  ...
  async max() {
    ...
    if (client) {
      console.info("Successful connection to Redis");
    } else {
